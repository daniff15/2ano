...
import miVersion.Alojamento;
import miVersion.Aventura;
import miVersion.BaseCatalogAdmin;
import miVersion.CatalogAdmin;
import miVersion.CatalogStats;
import miVersion.MundoRural;
import miVersion.PacoteServicos;
import miVersion.Passeio;
import miVersion.TipoServico;
import miVersion.Transporte;
...
public class SenteNatureza {
	...
	private static void alineaD(PrintStream out) {
		...
		CatalogAdmin ctgAdmin1 = new BaseCatalogAdmin();
		CatalogAdmin ctgAdmin2 = new BaseCatalogAdmin();

		lg.registarServico(ctgAdmin1, "1001",
				new Alojamento("Camping Serr&Sol - Tenda 1", "Local para tenda, 2 m2", 40, 1));
		lg.registarServico(ctgAdmin1, "1002",
				new Alojamento("Camping Serr&Sol - Tenda 2", "Local para tenda, 3 m2", 50, 2));
		lg.registarServico(ctgAdmin2, "1003",
				new Alojamento("Camping Serr&Sol - Tenda 3", "Local para tenda, 5 m2", 70, 3));
		lg.registarServico(ctgAdmin2, "1004", new Alojamento("Camping Serr&Sol - Bungalow 1", "Bungalow T0", 90, 2));
		lg.registarServico(ctgAdmin1, "1005", new Alojamento("Camping Serr&Sol - Bungalow 2", "Bungalow T1", 120, 3));

		lg.removerServico(ctgAdmin1, "1005");
		lg.removerServico(ctgAdmin2, "1003");

		for (String s : lg.logs()) {
			out.println(s);
		}
